...
def main():
    ...
    for idx_net in range(num_exp):
        ...
        # for method in [ 'iDLG']:
            ...
            # print("dy_dx.shape:", len(dy_dx), len(dy_dx[6]),dy_dx[6][0].shape)
            #
            # for _ in dy_dx:
            #     print(len(_))
            #     print(_[0].shape)
            ...
            # return 0
            original_dy_dx = list((dy_dx[i].detach().clone() if i != 6 else (dy_dx[i]*0).detach().clone() for i in range(len(dy_dx))))  #这里最后一层的参数or梯度全部置为0
            # original_dy_dx = list((_.detach().clone() for _ in dy_dx))
