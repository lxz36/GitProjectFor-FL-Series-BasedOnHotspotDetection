...
def RecursionAddNoise(n):  # 通过递归的方式给不规则list列表中每个元素加噪声
    if type(n[0]) is list:
        ...
        noiseValue=np.random.laplace(0,0.1,list(n[0].shape))  #list(n[0].shape)  将torch的维度转化为list的维度
...
def main():
    ...
    num_exp = 3  #initial Value=1000  这里代表最终总共生成复原多少张图片  每次只恢复一张照片出效果是最好的；最好输入也只有一张照片；每个文件夹里面抽取num_exp张图片；然后恢复num_exp张图片
