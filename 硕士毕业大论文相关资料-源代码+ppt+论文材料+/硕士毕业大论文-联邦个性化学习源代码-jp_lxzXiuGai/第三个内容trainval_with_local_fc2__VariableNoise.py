...
for noiseVariance in noiseVariances:
    ...
    for server_round in range(max_round):
        ...
        for client_idx in client_selected:  # 对每一个client开启一个对话
            ...
            '''
            局部优化后就进行模型测试  Test clients
            '''

            if (client_idx == 0 ): #test_data_key == 'asml1'的情况
                outSum = test(model=client_model,
                              data_loader=test_loader[0],  #test_loader[0]表示iccad测试集
                              criterion=criterion,
                              batch_size=test_batchsize,
                              display_step=display_step,
                              device=device)
            ...
            if (client_idx == clients_num // 2  ):  #test_data_key == 'asml1'的情况
                outSum = test(model=client_model,
                              data_loader=test_loader[1], #test_loader[1]表示asml1测试集
                              criterion=criterion,
                              batch_size=test_batchsize,
                              display_step=display_step,
                              device=device)
...
fileObject = open('第三个内容trainval_global_HFL-LA_VariableNoise.json', 'w', encoding="utf8")
