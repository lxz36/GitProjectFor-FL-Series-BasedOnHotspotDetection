...
arg_parser.add_argument("--n_iccad2012", type=int, default=0)
arg_parser.add_argument("--n_asml1", type=int, default=0)
arg_parser.add_argument("--sel_ratio", type=float, default=1.)
arg_parser.add_argument("--fc1-size", type=int, nargs='+')
...
assert len(args.fc1_size) == args.n_iccad2012 + args.n_asml1
...
client_models = [
    Model(n_features=n_features, fc1_size=fc1).to(device) for fc1 in args.fc1_size
]
client_states = [
...
for server_round in range(max_round):
    ...
    for client_idx in client_selected:  # 对每一个client开启一个对话
        ...
        restore_from_server(client_model, server_model, names_not_restore=['fc1.0', 'final_fc'])
    ...
    for _lr_sch in client_lr_schedulers:
        _lr_sch.step()
    print('\nStart Fed Avg...')
    fed_avg(client_states, server_model, names_not_merge=['fc1.0', 'final_fc'])
