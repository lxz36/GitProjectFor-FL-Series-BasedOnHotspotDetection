...
arg_parser.add_argument("--n_iccad2012", type=int, default=2)#1、初始default=0
arg_parser.add_argument("--n_asml1", type=int, default=2)#2、初始default=0
arg_parser.add_argument("--sel_ratio", type=float, default=1.)
...
# read .ini file
client_train_benchmark_path = infile.get('dir', 'client_benchmark') # benchmark dir for training
model_path = infile.get('dir','model_path')
server_model_path = infile.get('dir', 'server_model_path')
...
'''
max_itr_ds = {
    'iccad2012': 800,  #加快训练'iccad2012': 800,
    'asml1': 800}      #加快训练'asml1': 800
weight_decay_ds = {
    'iccad2012': 1e-5,
    'asml1': 1e-5}
...
#从这里开始噪声的选择：
# noiseVariances=[0,0.0001,0.001,0.01,0.1];  #几个噪声的方差值
noiseVariances=[0.1];  #几个噪声的方差值(用于噪声变化)
...
for noiseVariance in noiseVariances:
    ...
    # client_lr_init = [lr_init for i in range(clients_num)]
    ...
    client_optimizers = [
        optim.Adam(client_models[i].parameters(),
                   lr=lr_init,
                   betas=(.9, .999))
        # optim.SGD(client_models[i].parameters(),
        #           lr=lr_init,
        #           momentum=0.9)
        for i in range(clients_num)
    ]
    ...
    for server_round in range(max_round):
        ...
        fed_avgWithNoise(client_states, server_model,[],True,0,noiseVariance*(0.1)*pow(0.1,server_round//10))
        ...
        for i, test_data_key in enumerate(test_data_keys):
            ...
            if(test_data_key=="iccad2012"):
                if("初始噪声方差为%f的可变噪声实验:"%(noiseVariance) in ResultOfICCAD):
                    ResultOfICCAD["初始噪声方差为%f的可变噪声实验:"%(noiseVariance)].append(outSum)
                    accResultOfICCAD["初始噪声方差为%f的可变噪声实验的精度acc变化:" % (noiseVariance)].append(outSum[1])
                else:
                    ResultOfICCAD["初始噪声方差为%f的可变噪声实验:" % (noiseVariance)]=[]
                    ResultOfICCAD["初始噪声方差为%f的可变噪声实验:" % (noiseVariance)].append(outSum)

                    accResultOfICCAD["初始噪声方差为%f的可变噪声实验的精度acc变化:" % (noiseVariance)] = []
                    accResultOfICCAD["初始噪声方差为%f的可变噪声实验的精度acc变化:" % (noiseVariance)].append(outSum[1])


            if(test_data_key=="asml1"):
                if("初始噪声方差为%f的可变噪声实验:"%(noiseVariance) in ResultOfAsml):
                    ResultOfAsml["初始噪声方差为%f的可变噪声实验:"%(noiseVariance)].append(outSum)
                    accResultOfAsml["初始噪声方差为%f的可变噪声实验的精度acc变化:" % (noiseVariance)].append(outSum[1])
                else:
                    ResultOfAsml["初始噪声方差为%f的可变噪声实验:" % (noiseVariance)]=[]
                    ResultOfAsml["初始噪声方差为%f的可变噪声实验:" % (noiseVariance)].append(outSum)

                    accResultOfAsml["初始噪声方差为%f的可变噪声实验的精度acc变化:" % (noiseVariance)] = []
                    accResultOfAsml["初始噪声方差为%f的可变噪声实验的精度acc变化:" % (noiseVariance)].append(outSum[1])
