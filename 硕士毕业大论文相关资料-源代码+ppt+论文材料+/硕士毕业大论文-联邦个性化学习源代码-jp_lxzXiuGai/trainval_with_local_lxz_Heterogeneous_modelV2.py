...
from model_pytorch import Model1  # 引入我们自己构建的神经网络
from model_pytorch import Model  # 引入我们自己构建的神经网络
from data_pytorch import alloc_benchmark
from tools import *

arg_parser = argparse.ArgumentParser()
arg_parser.add_argument("--n_iccad2012", type=int, default=5,  # 1、初始default=0
                        help='number of iccad2012 clients')
arg_parser.add_argument("--n_asml1", type=int, default=5,  # 2、初始default=0
                        help='number of asml1 clients')
arg_parser.add_argument("--sel_ratio", type=float, default=0.5,
...
np.random.seed(
    42)  # 只能是一次有效,seed( ) 用于指定随机数生成时所用算法开始的整数值，如果使用相同的seed( )值，则每次生成的随即数都相同，如果不设置这个值，则系统根据时间来自己选择这个值，此时每次生成的随机数因时间差异而不同。
...
client_model_paths = [
    # ['./models/model\\client_asml1_0', './models/model\\client_asml1_1', './models/model\\client_asml1_2', './models/model\\client_asml1_3', './models/model\\client_asml1_4', './models/model\\client_iccad2012_0', './models/model\\client_iccad2012_1', './models/model\\client_iccad2012_2', './models/model\\client_iccad2012_3', './models/model\\client_iccad2012_4']
...
print('client benchmark path:',
      client_train_paths)  # client_train_paths ={'asml1': './benchmarks\\asml1_train', 'asml2': './benchmarks\\asml2_train', 'asml3': './benchmarks\\asml3_train', 'asml4': './benchmarks\\asml4_train', 'iccad2012': './benchmarks\\iccad2012_train'}
...
test_data_paths = {
    # test_data_paths = {'asml1': './wwy/benchmarks/asml1/test', 'iccad2012': './wwy/benchmarks/iccad_2012/test'}
...
max_itr_local = sum([[max_itr_local_ds[_ds]] * clients_num_per_ds[_ds]
                     # max_itr_local =[500, 500, 500, 500, 500, 500, 500, 500, 500, 500]
                     for _ds in ds_keys if _ds in max_itr_local_ds], [])
max_itr_global = sum([[max_itr_global_ds[_ds]] * clients_num_per_ds[_ds]
                      # max_itr_global = [1500, 1500, 1500, 1500, 1500, 1500, 1500, 1500, 1500, 1500]
...
weight_decay = sum([[weight_decay_ds[_ds]] * clients_num_per_ds[_ds]
                    # 权重的衰变weight_decay =[1e-05, 1e-05, 1e-05, 1e-05, 1e-05, 1e-05, 1e-05, 1e-05, 1e-05, 1e-05]
...
sel_ch = [0, 12, 4, 27, 3, 29, 17, 2, 21, 8, 28, 30, 22, 19, 24, 25, 16, 20, 5, 11, 31, 9, 13, 26, 18, 7, 23, 15, 1, 14,
          10, 6]
...
server_model = Model(n_features=n_features).to(device)  # revise1这里需要修改一下  中央服务器用的两层神经网络
server_avg_Model = Model(n_features=n_features).to(device)
iccad_model = Model(n_features=n_features).to(device)  #
asml1_model = Model1(n_features=n_features).to(device)  #

client_models = [copy.deepcopy(asml1_model).to(device) if i < args.n_asml1 else copy.deepcopy(iccad_model).to(device)
                 for i in range(clients_num)]  # revise2
...
# 需要聚合的全局参数名字集合
avg_name_sum = ['conv1_1.0.weight', 'conv1_1.0.bias', 'conv1_2.0.weight', 'conv1_2.0.bias', 'conv2_1.0.weight',
                'conv2_1.0.bias', 'conv2_2.0.weight', 'conv2_2.0.bias', 'fc1.0.weight', 'fc1.0.bias']
for server_round in range(max_round):
    ...
    for client_idx in client_selected:  # 对每一个client开启一个对话
        ...
        # 加载全局参数 分发全局参数
        client_models_copy = client_model.state_dict()  # 每个本地模型的copy
        for name in avg_name_sum:
            client_models_copy[name] = server_avg_Model.state_dict()[name]

        client_model.load_state_dict(client_models_copy)
        # restore_from_server(client_model, server_model, names_not_restore=['final_fc']) #revise5

        '''
        Train client for one round (when reaches max iteration steps)
        '''
        print('Local layers training...')
        train(model=client_model,  # revise6 不需要改，因为全局层是固定的
              ...
              device=device,
              untrained_modules=[
                  'conv1_1', 'conv1_2', 'conv2_1', 'conv2_2', 'fc1'])
        # update steps
    ...
    print('\nStart Fed Avg...')
    # 做全局参数聚合
    server_avg_Model_copy = server_avg_Model.state_dict()
    for name in avg_name_sum:
        avg_flag = 0
        for client_idx in client_selected:
            if avg_flag == 0:
                avg_flag = 1
                server_avg_Model_copy[name] = client_states[client_idx]['model'][name] / selected_client_num
            else:
                server_avg_Model_copy[name] += (client_states[client_idx]['model'][name] / selected_client_num)

    server_avg_Model.load_state_dict(server_avg_Model_copy)  # 这里才能将参数真正的加载进去，聚合后的模型是server_avg_Model

    # fed_avg(client_states, server_model)  #关键语句
