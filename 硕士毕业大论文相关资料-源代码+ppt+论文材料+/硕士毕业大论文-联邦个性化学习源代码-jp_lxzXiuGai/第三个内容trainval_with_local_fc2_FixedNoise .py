...
#从这里开始噪声的选择：
noiseVariances=[0.0,0.0001,0.001,0.01,0.1];  #几个噪声的方差值
...
for noiseVariance in noiseVariances:
    ...
    for server_round in range(max_round):
        ...
        fed_avgWithNoise(client_states, server_model, [], True, 0, noiseVariance)  #关键语句
        ...
        for i, test_data_key in enumerate(test_data_keys):
            for client_idx, client_model in enumerate(client_models):
                ...
                if(client_idx==0 and test_data_key=='asml1'):
                ...
                if(client_idx==clients_num//2 and test_data_key=='iccad2012'):
                    outSum=test(model=client_model,
                        data_loader=test_loader[i],
                        criterion=criterion,
                        batch_size=test_batchsize,
...
fileObject = open('第三个内容trainval_global_HFL-LA_FixedNoise.json', 'w', encoding="utf8")
