...
#这是目前这个文件夹下相关程序的模板；有标准程序、优化程序、程序注释等（用的是中间层做个性化层）
import sys
import os
import random
import time
    ...
    for i_Ratio in range(len(RatioSum)):
        ...
        for server_round in range(max_round):
            ...
            for client_idx in client_selected:  # 对每一个client开启一个对话
                ...
                #观察client_model层的名字（'conv1_1' ,'conv1_2', 'conv2_1', 'conv2_2', 'fc1','final_fc'）和架构
                ...
                restore_from_server(client_model, server_model, names_not_restore=[  'conv1_2', 'conv2_1', 'conv2_2', 'fc1']) #revise5.1个性化层
                ...
                train(model=client_model,    #revise6
                      ...
                      device=device,
                      untrained_modules=[          #revise5.2全局层
                          'conv1_1' ,'final_fc'])
                # update steps
            ...
            for _lr_sch in client_lr_schedulers:
                _lr_sch.step()
            print('\nStart Fed Avg...')
            fed_avg(client_states, server_model, names_not_merge=['conv1_2', 'conv2_1', 'conv2_2', 'fc1'])  #关键语句revise5.3全局层 names_not_merge=个性化层
