...
#这个模型是基于卷积层惩罚项
class FedMD_BaseConvPunishment():
    ...
    def __init__(self, parties, public_dataset,
        ...
        self.FedMD_lambda_reg=FedMD_lambda_reg
    ...
    def collaborative_training(self):  # 开始联邦蒸馏学习，异步从这里改造
        ...
        while True:
            ...
            # 将聚合后的卷积层参数conv1_weights_average赋值给每一个模型,这一步可以优化一下
            # 遍历每个模型
            for d in self.collaborative_parties:
                # 提取模型的权重
            ...
            # 定义自定义的损失函数，加入卷积层参数差值的惩罚项
            def custom_loss(y_true, y_pred, model_weights, conv1_weights_average, conv1_bias_average, lambda_reg=0.01):
                mse_loss = tf.keras.losses.MeanSquaredError()(y_true, y_pred)
                reg_loss = tf.reduce_sum(tf.square(model_weights[0] - conv1_weights_average)) + \
                           tf.reduce_sum(tf.square(model_weights[1] - conv1_bias_average))
...
'''
                    ...
                    # 编译模型，使用自定义损失函数，在蒸馏模型中，全局共识logits相当于真实标签
                    y_pred = d["model_logits"].predict(alignment_data["X"], verbose=0)
                    FedMD_lambda_reg = self.FedMD_lambda_reg
                    # 创建部分应用的自定义损失函数 ,partial将 model_weights, conv1_weights_average, conv1_bias_average, lambda_reg 四个参数固定住，只保留 y_true, y_pred 这两个参数
                    loss_fn = partial(custom_loss,
                                      model_weights=d["model_weights"],
                                      conv1_weights_average=conv1_weights_average,
                                      conv1_bias_average=conv1_bias_average,
                                      lambda_reg=FedMD_lambda_reg)

                    d["model_logits"].compile(optimizer='adam', loss=lambda logits, y_pred : loss_fn(logits, y_pred))
