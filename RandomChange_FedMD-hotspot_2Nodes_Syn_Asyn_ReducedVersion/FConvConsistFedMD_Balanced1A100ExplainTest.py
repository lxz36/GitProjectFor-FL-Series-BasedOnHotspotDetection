...
if __name__ == "__main__":
    ...
    dataSet_reduct_para = 2  #lxztodo可以弄到公共参数配置文件里面取 可以分2份数据出来 dataSet_reduct_para>=2，目前还不使用这个参数，dataSet_reduct_para = 1代表取全部数据
    # iccad训练集，取第一部分
    ...
    for asynchronousRate in asynchronousRates:
        if model_saved_dir is not None:
            ...
            print("按照顺序读取和训练模型，一定要对准model_names", model_names)
