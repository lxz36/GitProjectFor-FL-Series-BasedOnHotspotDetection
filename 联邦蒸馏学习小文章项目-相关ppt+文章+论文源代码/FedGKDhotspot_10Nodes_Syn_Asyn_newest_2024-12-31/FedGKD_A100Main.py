...
from FedGKD  import  FedGKD

def parseArg():
    parser = argparse.ArgumentParser(description='FedGKD, a federated learning framework. \
    Participants are training collaboratively. ')  # FedGKD，一个联邦学习框架。参与者正在合作培训。
    parser.add_argument('-conf', metavar='conf_file', nargs=1,
                        help='the config file for FedGKD.'
...
if __name__ == "__main__":
    ...
    # asml1训练集，取第一部分
    X_train_ConvConsistFedMD_asml1 = X_train_ConvConsistFedMD_asml1[:X_train_ConvConsistFedMD_asml1.shape[0] // dataSet_reduct_para, :, :]
    y_train_ConvConsistFedMD_asml1 = y_train_ConvConsistFedMD_asml1[:y_train_ConvConsistFedMD_asml1.shape[0] // dataSet_reduct_para]
    ...
    for asynchronousRate in asynchronousRates:
        ...
        initialization_result = FedGKD.init_result
        pooled_train_result = FedGKD.pooled_train_result

        collaboration_performance = FedGKD.collaborative_training()
