...
def load_ConvConsistFedMD_data(file, verbose = False, standarized = False):  #载入ConvConsistFedMD数据
    """
    file should be the downloaded ConvConsistFedMD file in .mat format.
    """    
    ...
    if verbose == True:     #输出数据集信息
        print("ConvConsistFedMD-letter dataset ... ")
        print("X_train shape :", X_train.shape)
        print("X_test shape :", X_test.shape)
        print("y_train shape :", y_train.shape)
        print("y_test shape :", y_test.shape)
...
def generate_partial_data(X, y, class_in_use = "all", verbose = False):#生成部分数据
    if class_in_use == "all":
...
def generate_bal_private_data(X, y, N_parties = 10, classes_in_use = range(11),
                              N_samples_per_class = 20, data_overlap = False):   #生成bal私有数据
    """
    Input: 
    -- N_parties : int, number of collaboraters in this activity;
    -- classes_in_use: array or generator, the classes of ConvConsistFedMD-letters dataset
    (0 <= y <= 25) to be used as private data; 
    -- N_sample_per_class: int, the number of private data points of each class for each party
    输入：
     -- N_parties : int，本次活动的节点数量；
     -- classes_in_use: 数组或生成器，ConvConsistFedMD-letters 数据集的类 10个节点私有数据和所有数据总和
     (0 <= y <= 25) 用作私有数据；
     -- N_sample_per_class: int，每一个节点每一类的私有数据样本数量
    return: 
