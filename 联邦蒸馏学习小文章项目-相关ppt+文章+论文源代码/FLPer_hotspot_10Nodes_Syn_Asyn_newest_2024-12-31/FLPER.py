...
class FLPER():
    ...
    def collaborative_training(self):  # 开始联邦学习，异步从这里改造
        ...
        while True:
            ...
            # 将聚合后的参数赋值给每一个模型的前4层卷积层
            for d in self.collaborative_parties:
                # 提取模型的权重 weights_average 和d["model_weights"]维度一样
                weights = d["model_weights"]
                # 提取卷积层的权重
                for i in range(24):  # 卷积层一共4层，一层对应6个list参数
                    weights[i] = weights_average[i].copy()  # 深度复制
                # 其他层不做覆盖
            ########## 测试per
